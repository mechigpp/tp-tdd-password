...
test("si la contraseña tiene 8 caracteres o mas, pasa", () => {
    const resultado = validarPassword("Mercedespspsps1-");
...
test("si la contraseña no tiene minimo 1 letra en mayuscula, falla", () => {
    const resultado = validarPassword("mechpspsps1-");
...
test("si la contraseña no tiene minimo 1 letra en minuscula, falla", () => {
    const resultado = validarPassword("MECHIPSPSPS1-");
...
test("si la contraseña  tiene minimo 1 letra en minuscula, pasa", () => {
    const resultado = validarPassword("Mechpsppsps1-");
...
test("si la contraseña no tiene minimo 1 numero, falla", () => {
    const resultado = validarPassword("Mechpspsps-");
...
test("si la contraseña tiene 3 caracteres iguales consecutivos, falla", () => {
    const resultado = validarPassword("Mechiii123!");

    expect(resultado.esValida).toBe(false);
    expect(resultado.errores).toContain("ERROR: no debe contener 3 caracteres iguales consecutivos");
});

test("si la contraseña no tiene 3 caracteres iguales consecutivos, pasa", () => {
    const resultado = validarPassword("Mechi123!");
